...
text_layout(
    ...
    if (have_raqm && family->layout_engine == LAYOUT_RAQM) {
        count = text_layout_raqm(
            string, family, dir, features, lang, glyph_info);
    } else
#endif
    {
...
static int
bounding_box_and_anchors(FontFamily *family, const char *anchor, int horizontal_dir, GlyphInfo *glyph_info, size_t count, int load_flags, int *width, int *height, int *x_offset, int *y_offset) {
...
text_getsize(FontFamily *family, PyObject *args) {
    ...
    error = bounding_box_and_anchors(family, anchor, horizontal_dir, glyph_info, count, load_flags, &width, &height, &x_offset, &y_offset);
    if (glyph_info) {
        PyMem_Free(glyph_info);
        glyph_info = NULL;
    }
