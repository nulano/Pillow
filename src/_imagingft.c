...
static int
bounding_box_and_anchors(void *self, int is_font_family, const char *anchor, int horizontal_dir, GlyphInfo *glyph_info, size_t count, int load_flags, int *width, int *height, int *x_offset, int *y_offset) {
...
text_getsize(void *self, int is_font_family, PyObject *args) {
    ...
    error = bounding_box_and_anchors(self, is_font_family, anchor, horizontal_dir, glyph_info, count, load_flags, &width, &height, &x_offset, &y_offset);
    if (glyph_info) {
        PyMem_Free(glyph_info);
        glyph_info = NULL;
    }
